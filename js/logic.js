...
(function (util, EventUtil, cookie, ajax) {
	...
	var coursesObj = {},	 // 保存请求课程列表时返回数据转化后的对象
		...
		pSize, type = 10, 	 // pSize为单页课程列表的课程数，type为课程的类型
	...
	/* 风采展示区滚动动画 */
	function moveShow (container, interval, moveTime) {
		container.style.left = '0px';
		var distance = $$('.showPic', container)[0].offsetWidth,	
			stepLen = Math.floor(distance/(moveTime/interval)),
			pic1, len, flag = 1;
		var step = function () {
			len = Math.abs(parseInt(container.style.left)) + stepLen;
			if (len < distance ) {
				container.style.left = '-' + len + 'px';
				if (flag == 1) {
					flag = 0;
					pic1 = $$('.showPic', container)[0];
					container.appendChild(pic1.cloneNode(false));
				}
			} else {
				clearInterval(intervalId);	
				flag = 1;		
				container.removeChild(pic1);
				container.style.left = '0px';
	...
	PageMod.prototype.pgUp = function () {
		...
		if ((index > 1) && (index <= this.num)) {
			...
			getCourses(this.index, pSize, this.type);
		}
	};
	/* 为分页器添加切换下一页方法 */
	...
	addHandler(document, 'DOMContentLoaded', function (event) {			
		...
		/* 为浏览器窗口宽高发生变化添加resize事件 */	
		(function () {
			var timeoutId;
			addHandler(window, 'resize', function (event) {
		...
		/* 滚动风采展示区 */
		var picCont = $('#show_area .picCont');
		moveShow(picCont, 30, 9600);
